...
async function addUserProfile(userId, firstName, lastName, gender, sexualPreference, biography) {
    const query =
        "INSERT INTO profiles(id, first_name, last_name, gender, sexual_preference, biography) VALUES($1, $2, $3, $4, $5, $6) RETURNING *";
    const data = await client.query(query, [userId, firstName, lastName, gender, sexualPreference, biography]);
    if (data.rowCount == 1) {
        return data.rows[0];
    } else {
        return null;
