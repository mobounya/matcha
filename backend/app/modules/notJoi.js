...
class CustomValidator {
    ...
    string() {
        ...
        function password() {
            ...
            function isValidPassword(value) {
                const passwordRegex = /^.*(?=.{8,})(?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=-_]).*$/;
                if (
                    value.length < 8
                ) {
                    return {
                        error: 'Your password must be at least 8 characters long. Please try another.'
                    }
                } else if (
                    value.length > 64
                ) {
                    return {
                        error: 'Your password is must had a maximum length of 64, please try another.'
                    }
                } else if (!value.match(passwordRegex)) {
                    return {
                        error: 'Please choose a more secure password. Try a mix of letters, numbers and symbols.'
                    }
                }
                return { value };
            }
    ...
    object(schemaObj) {
        function validateObject(value) {
            ...
            return errors.length > 0 ? { value: newValue, errors } : { value: newValue };
        }
        return {
            type: "object",
            types: this.types,
